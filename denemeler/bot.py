...
# Kod başlatıldığında terminale botun çalıştığını yaz
@bot.event
async def on_ready():
...
# info komutu ekler kullanıcılara komutlar tanıtılır
...
# Veri tabanına yeni proje ekleme
...
# Projen varsa onu gösterme
@bot.command(name='projects')
async def get_projects(ctx):
    user_id = ctx.author.id
    projects = manager.get_projects(user_id) # Projenin user_id sini al
    if projects:
...
# İstediğin projeye yetenek eklemek
...
async def delete_project(ctx):
    ...
    if projects:
        ...
        manager.delete_project(user_id, project_id) # İstenilen projeyi siler
        await ctx.send(f'{project_name.content} projesi veri tabanından silindi!')
    else:
        await ctx.send('Henüz herhangi bir projeniz yok!\nBir tane eklemeyi düşünün! !new_project komutunu kullanabilirsiniz.')

# İstediğin projeyi güncellemek
